      ...
      for (let x = 0; x < canvasWidth; x += 8) {
        ...
        if (noiseVal > threshold) {
          ...
          const intensity = Math.min(
            0.4,
            Math.max(0.1, 0.1 + totalInfluence * 0.2)
          );
    ...
    if (!removeWaveLine) {
      ripples.current.forEach((ripple) => {
        ...
        if (age < maxAge) {
          ...
          const alpha = (1 - progress) * 0.1 * ripple.intensity;
